...
const Login = () => {
  ...
  const isLikelyEmail = (input) => {
    input = input.trim();
    return isNaN(input[0]) || input.includes("@");
  };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const payload = isEmail
        ...
        : {
            mobile: `${formData.countryCode}${identifier}`,
          ...
          <div className="flex gap-2">
            {showCountryCode && (
              <select
                ...
                className="w-1/3 rounded-md bg-slate-700 border border-slate-600 px-3 py-3 text-slate-200 focus:outline-none focus:ring-2 focus:ring-cyan-400"
              >
